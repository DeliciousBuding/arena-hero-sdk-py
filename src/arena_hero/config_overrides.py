...
def _coerce_to(existing: object, override: object) -> object:
...
def _deep_merge(target: object, override: object) -> object:
    """深合并:两者都是 dict 时递归合并;其余类型整体替换。"""

    if isinstance(target, dict) and isinstance(override, dict):
        merged: dict[Any, Any] = dict(target.items())
...
def _walk_setattr(
    ...
    setattr(
        final_object, final_name, _deep_merge(existing, _coerce_to(existing, value))
    )
    return True
